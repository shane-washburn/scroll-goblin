...
export async function llmMove(state: ChaosState, faction: Faction, lastError?: string, context?: ChaosMemory & { human_cheats_remaining: number }) {
  try {
    return await post<ChaosAction>(`${import.meta.env.VITE_API_BASE_URL ?? 'http://localhost:8787'}/goblin-chess/v1/chaos`, { pieces: state.pieces, captured: state.captured, turn: state.turn, faction, round: Math.floor(state.ply / 2), lastError, ...context });
  } catch (error) {
