...
export default function Playhouse(){
 ...
 const choose=(id:string)=>{if(selected===id){dismiss();return;}setSelected(id);setMoving(!placements[id]);void playCue('pick');setNotice(placements[id]?'Choose move, rotate, or return to basket.':`${items.find(i=>i.id===id)!.name} selected. Choose a glowing spot.`);};
 ...
 return <main className="lp">
  ...
  <section className="lp-inventory" aria-label="Toy basket"><span className="lp-basket" aria-hidden="true">🧺</span><div className="lp-ribbon">{items.filter(item=>filter==='all'||item.categories.some(c=>c===filter)).map(item=><button key={item.id} className={`${selected===item.id?'chosen':''} ${placements[item.id]?'placed':''}`} aria-label={`${item.name}${placements[item.id]?', in the room':''}`} aria-pressed={selected===item.id} onClick={()=>choose(item.id)}>{thumbs[item.id]?<img src={thumbs[item.id]} alt=""/>:<span aria-hidden="true">{item.icon}</span>}{placements[item.id]&&<small aria-hidden="true">✓</small>}</button>)}</div></section>
